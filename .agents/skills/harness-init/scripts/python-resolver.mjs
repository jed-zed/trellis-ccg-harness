...
function locate(candidate, platform, runner) {
  const result = runner(
    platform === "win32" ? candidate.command : "which",
    platform === "win32"
      ? [...candidate.argsPrefix, "-I", "-S", "-c", "import sys; print(sys.executable)"]
      : [candidate.command],
  );
  const first = String(result.stdout ?? "")
    ...
    .map((line) => line.trim())
    .find(Boolean);
  return result.status === 0 && first
    ? { command: first, argsPrefix: platform === "win32" ? [] : candidate.argsPrefix }
    : candidate;
}
...
export function resolvePython(options = {}) {
  ...
  for (const candidate of candidates) {
    ...
    if (result.status === 0 && version && isSupported(version)) {
      return {
        ...locate(candidate, platform, runner),
        version,
      };
    }
  }
