...
- **R5 — 支持的 lifecycle**：先以 `2052120` 修复 update 对未发布目标 runtime 的错误前置依赖，将 snapshot update 与 runtime install 解耦，并同步对应测试和规范；随后运行 `pnpm harness:update -- --ccg-commit <40-sha> --source-checkout <clean-checkout>` 生成 CCG snapshot 和 `harness.sources.json`。不得手工编辑 snapshot、manifest、ownership 或 managed block；Trellis 保持 `0.6.9`。
...
- [x] **AC2 / R5**：PR #44 包含前置 lifecycle 修复 `2052120`（将 snapshot update 与 runtime install 解耦）及其测试、规范和 README，并包含 lifecycle 生成的必要 snapshot、manifest 与受管版本表面；Trellis 仍为 `0.6.9`。
