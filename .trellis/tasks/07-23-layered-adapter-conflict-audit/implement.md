...
- [ ] Move `grill-me` into the 14 bundled public Harness platform Skills and
...
- [ ] Publish the existing 45-Skill repository as an authenticated private
...
- [ ] Add a guided setup contract: prefer the host's native structured-choice
  ...
  ambiguity.
- [ ] Implement a resumable Global Init command that inventories installed and
...
- [ ] Add read-only status adapters for Codex, Gemini, Grok, and Claude Code
...
- [ ] Put every network, install, and login operation behind its own native-UI
...
- [ ] Default Claude Code to skipped and label its install/login choice as
...
- [ ] Implement Project Init as a separate phase: discover stack evidence,
  ...
  `mark-ready`.
- [ ] Add focused tests and documentation for clean global platform projection,
...
- [ ] Run all Harness/CCG quality, security, provenance, doctor, context, and
