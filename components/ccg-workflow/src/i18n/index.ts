...
const zhCN = {
  ...
  cli: {
    help: {
      ...
      optionDescriptions: {
        ...
        searchModels: '搜索模型（逗号分隔）',
        collaborationMode: '协作模式 (parallel/smart/sequential)',
        workflows: '要安装的工作流',
        installDir: '安装目录',
        enableIntelligence: '非交互模式下显式启用 Grok 外部情报层',
        disableIntelligence: '显式禁用 Grok 外部情报层',
      },
  ...
  init: {
    ...
    selectSearchModels: '选择搜索任务使用的模型（可多选）',
    selectMode: '选择协作模式',
    ...
    summaryMenu: {
      ...
      editMcp: '改 MCP 工具',
      editPerf: '改性能模式',
      editIntelligence: '改外部情报设置',
      cancel: '取消安装',
    },
    intelligence: {
      title: 'Grok 外部情报层',
      prompt: '是否启用自动联网搜索与证据收集？',
      enableOption: '启用（主编排器按任务需要自动判断是否搜索）',
      disableOption: '禁用（默认，不向 Grok 发送任务内容）',
      disclosureData: '发送内容：当前任务文本与聚焦后的源码快照，不发送整个工作区。',
      disclosureSearch: '搜索能力：可调用 Web 搜索；X 证据仅在工具事件返回真实来源时采信。',
      disclosureCost: '费用：Grok 运行会消耗模型 token 和工具调用额度。',
      disclosureArtifacts: '本地证据产物：{{path}}。',
      disclosureLogin: '认证：首次使用通过官方浏览器登录；凭据隔离在 {{path}}。',
      disclosureSafety: '安全：权限请求、来源缺失或协议异常时失败关闭，不降级到旧搜索提供方。',
      disclosureInit: '本次初始化不会发起登录，也不会执行付费搜索冒烟测试。',
    },
    mcp: {
      title: 'MCP 代码检索工具配置',
...
const en: typeof zhCN = {
  ...
  cli: {
    help: {
      ...
      optionDescriptions: {
        ...
        searchModels: 'Search models (comma-separated)',
        collaborationMode: 'Collaboration mode (parallel/smart/sequential)',
        workflows: 'Workflows to install',
        installDir: 'Installation directory',
        enableIntelligence: 'Explicitly enable Grok external intelligence in non-interactive mode',
        disableIntelligence: 'Explicitly disable Grok external intelligence',
      },
  ...
  init: {
    ...
    selectSearchModels: 'Select models for search tasks (multi-select)',
    ...
    summaryMenu: {
      ...
      editIntelligence: 'Edit external intelligence',
      cancel: 'Cancel installation',
    },
    intelligence: {
      title: 'Grok External Intelligence',
      prompt: 'Enable automatic web search and evidence collection?',
      enableOption: 'Enable (the orchestrator decides when a task needs search)',
      disableOption: 'Disable (default; no task content is sent to Grok)',
      disclosureData: 'Data sent: task text and a focused source snapshot, never the whole workspace.',
      disclosureSearch: 'Search: Web search is available; X evidence is trusted only when tool events return the source.',
      disclosureCost: 'Cost: Grok runs consume model tokens and tool-call quota.',
      disclosureArtifacts: 'Local evidence artifacts: {{path}}.',
      disclosureLogin: 'Authentication: first use opens the official browser login; credentials are isolated in {{path}}.',
      disclosureSafety: 'Safety: permission requests, missing sources, or protocol errors fail closed with no legacy-provider fallback.',
      disclosureInit: 'Initialization performs no login and no paid search smoke test.',
    },
    mcp: {
    ...
    model: {
      ...
      selectSearch: 'Select search model',
