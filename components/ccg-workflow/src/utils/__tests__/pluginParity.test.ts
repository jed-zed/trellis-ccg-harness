...
describe('Codex plugin release parity', () => {
  ...
  it('pins external providers to the native-permission non-lite wrapper launch contract', () => {
    ...
    expect(review).toContain('--skip-permissions')
    expect(reviewHelper).toContain('copy_snapshot_tree')
    expect(reviewHelper).toContain('choices=("claude", "antigravity")')
    expect(reviewHelper).toContain('"--antigravity-review", "--skip-permissions"')
  ...
  it('keeps search and product-manager companion routing on every affected skill surface', () => {
    ...
    expect(routingRule).toMatch(/Search is\s+advisory companion evidence/)
    expect(routingRule).toContain('does not block an otherwise')
    expect(routingRule).toContain('`not_applicable` is valid when current external evidence')
