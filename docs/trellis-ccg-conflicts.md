...
| Claude product-manager boundary versus workspace ownership | Blocking | Claude may review product evidence, but must never inherit workspace or lifecycle authority | Fixed: Codex is the sole writer; Claude sees only a bounded snapshot with Read/Glob/Grep. Project transport defaults to local; explicit environment-only SSH v2 never falls back |
