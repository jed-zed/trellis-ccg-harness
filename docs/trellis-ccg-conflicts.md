...
| Personal source versus runtime implementation | Blocking only for source identity | `components/ccg-workflow/` is an exact personal Git tree, while installed CLI/plugin state is user-local | Fixed: the snapshot is provenance; any available personal CCG CLI/plugin version is runtime-compatible by owner policy |
| Provider role routing versus workspace ownership | Blocking | CCG can route any registered Provider CLI for `frontend`, `backend`, or `search`, while Harness must retain one workspace writer | Fixed: CCG owns role selection; the adapter blocks any non-Codex Provider from gaining workspace write access |
...
| Package and source-snapshot drift | Blocking | Trellis, pnpm, CCG component identity, and the personal Git tree have separate provenance sources | Fixed: the adapter audits tracked project assets against `harness.sources.json` and `.harness/adapter.json` |
| Installed personal CCG version differs from the source snapshot | No conflict | The personal CCG author and Harness owner maintain cross-version compatibility | The adapter verifies that the CLI/plugin is available and reports its observed version without comparing it to `harness.sources.json` |
...
| Grok runtime or external intelligence unavailable | Warning | Role routing depends on the local Provider CLI, while search-backed external intelligence has separate credentials and opt-in state | Deferred per capability: unavailable Grok work fails at its runtime boundary, while unrelated roles remain unblocked |
